...
def modality_barchart(dept):
    if dept is None:
        ...
        group_modalities = subset_df.groupby('Room').agg({'Class':'count', 'Size': 'sum', 'Max': 'sum'}).reset_index()
        ...
        group_modalities = subset_dff.groupby('Room').agg({'Class': 'count', 'Size': 'sum', 'Max': 'sum'}).reset_index()
        group_modalities['Fill'] = group_modalities['Size'] / group_modalities['Max']
        # group_modalities = group_modalities.iloc[1:5, :]
...
def modality_pie_chart(dept):
    if dept is None:
        ...
        fig = px.pie(group_modalities, values='Perc', names='Room')
        return fig
    else:
        subset_df = df[['Dept', 'Room', 'Class', 'Size', 'Max']]
        subset_dff = subset_df[subset_df['Dept'] == dept]
